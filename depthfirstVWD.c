...
int n_workers; /* Number of worker threads inherited from Wool */
int *totals; /* Array results for thread-local storage (TLS) */
...
/* Check character by character if "c" is a real match */
...
/* DepthFirstVWD search algorithm */
TASK_4(void*, df_vishkin_search, const char*, P, const char*, T, size_t, m, size_t, n) {
        ...
        *witness = -1;
        ...
        for (pos = 1; pos < witnessSize; pos++) {
                ...
                if (witness[pos] == -1) { /* In this stage of our investigation, we only work with non-periodic patterns */
...
TASK_2(int, main, int, argc, char**, argv) {
  ...
  for (ii = 0; ii < n; ii++) {
	...
	/* Call to DephFirstVWD algorithm using Wool syntax */
